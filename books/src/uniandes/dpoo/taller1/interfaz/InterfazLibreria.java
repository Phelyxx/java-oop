...
public class InterfazLibreria extends JFrame
{
	 ...
	 * @throws IOException 
	 */
	public void cargarArchivos(File archivo_categorias, File archivo_libros) throws IOException
	{
		try
		{			
		...
		catch (Exception e)
		{
			libreria = new Libreria(archivo_categorias.getPath(), archivo_libros.getPath());
			panelCategorias.actualizarCategorias(libreria.darCategorias());
			JOptionPane.showMessageDialog(this, "Las materias agregadas fueron: " + libreria.darNuevos());
			e.printStackTrace();
		}
	...
	public void eliminarLibrosAutores()
	{
		String autores = JOptionPane.showInputDialog(this, "Escriba el nombre de los autor(es) separados por coma", "autores");
		try
		{
			int librosEliminados = libreria.eliminarLibrosAutor(autores);
			JOptionPane.showMessageDialog(this, Integer.toString(librosEliminados) + " libro(s) eliminado(s)", "Eliminar libros de autores" ,
			JOptionPane.INFORMATION_MESSAGE);
		}
		catch (Exception e)
		{
			JOptionPane.showMessageDialog(this, e.getMessage(), "No existe un autor" ,
